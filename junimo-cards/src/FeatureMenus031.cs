...
// v0.3.1 functionality-first menus. Final art/layout pass intentionally comes later.
internal sealed class FeatureCardShopMenu031 : IClickableMenu
{
    ...
    internal FeatureCardShopMenu031(ModEntry mod)
    {
        Mod = mod;
    ...
    public override void receiveLeftClick(int x, int y, bool playSound = true)
    {
        ...
        {
            Game1.activeClickableMenu = new FeaturePackShopMenu031(Mod, this);
            Game1.playSound("bigSelect");
            return;
        }
        if (Collection.Contains(x, y))
        {
            Game1.activeClickableMenu = new FeatureCollectionMenu031(Mod, this);
            Game1.playSound("bigSelect");
            return;
        }
        if (Shelf.Contains(x, y))
        {
            Game1.activeClickableMenu = new FeatureSaleShelfMenu031(Mod, this);
            Game1.playSound("bigSelect");
            return;
        }
    ...
    {
        Mod.EnsureState();
        CardUi.Begin(b, this, "JUNIMO CARDS · 기능 검증", "v0.3.1 기능 고정 패스 · UI 디자인은 다음 단계");
        ...
        DrawHomeButton(b, Shelf, "판매 진열대", $"고정 {Mod.Config.SaleShelfSlots}슬롯\n진열가 조정\n하루 최대 {Mod.Config.MaxDailySales}장");
...
internal sealed class FeaturePackShopMenu031 : IClickableMenu
{
    private readonly ModEntry Mod;
    ...
    internal FeaturePackShopMenu031(ModEntry mod, IClickableMenu returnMenu)
    {
        Mod = mod;
    ...
    public override void receiveLeftClick(int x, int y, bool playSound = true)
    {
        ...
        {
            Game1.activeClickableMenu = new FeaturePackOpeningMenu031(Mod, this);
            return;
        }
    ...
    public override void draw(SpriteBatch b)
    {
        CardUi.Begin(b, this, "팩 구매", "Pelican Origins · 30종 · 1팩 5장");
...
internal sealed class FeaturePackOpeningMenu031 : IClickableMenu
{
    private readonly ModEntry Mod;
    private readonly IClickableMenu ReturnMenu;
    private readonly List<CardPull> Pulls;
    private readonly List<Rectangle> CardBounds = new();
    private readonly bool[] Revealed = new bool[5];
    private readonly Rectangle Done;
    private readonly string OpeningMessage;
    private int RevealedCount;

    internal FeaturePackOpeningMenu031(ModEntry mod, IClickableMenu returnMenu)
    {
        Mod = mod;
        ReturnMenu = returnMenu;
    ...
    public override void receiveLeftClick(int x, int y, bool playSound = true)
    {
        ...
        for (int i = 0; i < Pulls.Count && i < CardBounds.Count; i++)
        {
            if (Revealed[i] || !CardBounds[i].Contains(x, y))
                continue;

            Revealed[i] = true;
            RevealedCount++;
            CardDefinition? card = Mod.FindCard(Pulls[i].CardKey);
            int rank = card is null ? 0 : ModEntry.GetRarityRank(card.Rarity);
            Game1.playSound(rank >= 4 ? "reward" : rank >= 2 ? "newArtifact" : "cardboardBox");
            return;
        }

        if (RevealedCount >= Pulls.Count && Done.Contains(x, y))
            Game1.activeClickableMenu = new FeatureCollectionMenu031(Mod, ReturnMenu);
    }
    ...
    public override void draw(SpriteBatch b)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            if (i >= Pulls.Count || !Revealed[i])
            {
                ...
                CardUi.CenterText(b, Game1.smallFont, "클릭해서 공개", new Rectangle(r.X, r.Bottom + 6, r.Width, 28), CardUi.Green, 0.9f);
                continue;
            }
        ...
        CardUi.CenterText(b, Game1.smallFont,
            $"공개 {RevealedCount}/{Pulls.Count} · Rare 천장 {Mod.State.PacksSinceRare}/10 · 미개봉 팩 {Mod.State.UnopenedPacks}",
            new Rectangle(xPositionOnScreen + 100, yPositionOnScreen + 505, width - 200, 30), CardUi.Muted, 0.95f);
        if (RevealedCount >= Pulls.Count)
            CardUi.Button(b, Done, "컬렉션에서 확인", true, true);
        drawMouse(b);
    }
}

internal sealed class FeatureCollectionMenu031 : IClickableMenu
{
    private readonly ModEntry Mod;
    private readonly IClickableMenu ReturnMenu;
    ...
    private string Message = "카드를 선택하세요. 컬렉션 보관용 1장은 판매하지 않습니다.";
    private int Page;

    internal FeatureCollectionMenu031(ModEntry mod, IClickableMenu returnMenu, int targetSlot = -1)
    {
        Mod = mod;
        ReturnMenu = returnMenu;
    ...
    public override void draw(SpriteBatch b)
    {
        CardUi.Begin(b, this, "컬렉션", $"수집 {Mod.Core.UniqueCardCount()}/{Mod.Cards.Count}종 · 총 {Mod.Core.TotalOwnedCopies()}장 · 판매는 여분 카드만 가능");
        ...
        if (selected.Card is not null)
        {
            ...
            string[] lines =
            {
                ...
                $"판매 가능 여분  {CardShopRules.GetListableCount(Mod, selected.CollectionKey)}장",
                $"최고 진열가  {selected.Value:N0}G"
            };
            for (int i = 0; i < lines.Length; i++)
                CardUi.Text(b, lines[i], new Vector2(detail.X + 28, detail.Y + 68 + i * 28), i == 6 ? CardUi.Green : CardUi.Ink, 0.9f);
            CardUi.CenterText(b, Game1.smallFont, selected.Card.Flavor, new Rectangle(detail.X + 20, detail.Bottom - 55, detail.Width - 40, 38), CardUi.Muted, 0.82f);
        }

        int available = string.IsNullOrWhiteSpace(SelectedKey) ? 0 : CardShopRules.GetListableCount(Mod, SelectedKey);
        CardUi.Button(b, List,
            TargetSlot >= 0 ? $"{TargetSlot + 1}번 슬롯에 진열" : $"판매 진열 +1 ({available}장 가능)",
            available > 0, true);

        var bonus = Mod.Core.GetNextCollectionBonus();
        CardUi.Button(b, Bonus,
            bonus.Complete ? "컬렉션 보너스 완료" : $"보너스 {bonus.Required}종 → 팩 {bonus.Reward}개",
        ...
        CardUi.CenterText(b, Game1.smallFont, $"{FilterName(Filter)} · {rows.Count}개 변형/상태 · {Page + 1}/{maxPage + 1}",
            new Rectangle(xPositionOnScreen + 335, yPositionOnScreen + height - 65, 300, 42), CardUi.Muted, 0.9f);
        CardUi.CenterText(b, Game1.smallFont, Message,
            new Rectangle(xPositionOnScreen + 790, yPositionOnScreen + 675, 340, 28), CardUi.Muted, 0.82f);
        CardUi.Button(b, Back, "뒤로");
        drawMouse(b);
    }
...
internal sealed class FeatureSaleShelfMenu031 : IClickableMenu
{
    private readonly ModEntry Mod;
    private readonly IClickableMenu ReturnMenu;
    ...
    internal FeatureSaleShelfMenu031(ModEntry mod, IClickableMenu returnMenu)
    {
        Mod = mod;
        ReturnMenu = returnMenu;
        ...
        int sx = r.X + 55;
        int sy = r.Y + 150;
        int slotCount = Math.Min(8, Mod.Config.SaleShelfSlots);
    ...
    public override void receiveLeftClick(int x, int y, bool playSound = true)
    {
        ...
        {
            Game1.activeClickableMenu = new FeatureCollectionMenu031(Mod, this, SelectedSlot);
            return;
        }
    ...
    public override void draw(SpriteBatch b)
    {
        CardUi.Begin(b, this, "판매 진열대", $"{Mod.Config.SaleShelfSlots}개 고정 슬롯 · 컬렉션 1장은 항상 보존 · 하루 최대 {Mod.Config.MaxDailySales}장 판매");
        ...
        for (int i = 0; i < Slots.Count; i++)
        {
            ...
            CardUi.CenterText(b, Game1.smallFont,
                $"{ModEntry.RarityName(card.Rarity)} · {ModEntry.VariantName(variant)} · {condition}",
                new Rectangle(r.X + 10, r.Y + 62, r.Width - 20, 26), CardUi.RarityColor(card.Rarity), 0.78f);
            CardUi.CenterText(b, Game1.smallFont, $"진열가 {listing.Price:N0}G",
                new Rectangle(r.X + 10, r.Y + 97, r.Width - 20, 26), CardUi.Green, 0.9f);
            CardUi.CenterText(b, Game1.smallFont, $"예상 판매확률 {Mod.Core.GetSaleChance(listing) * 100:0}%",
                new Rectangle(r.X + 10, r.Y + 127, r.Width - 20, 24), CardUi.Muted, 0.78f);
        }
        ...
        CardUi.CenterText(b, Game1.smallFont, Message,
            new Rectangle(xPositionOnScreen + 785, yPositionOnScreen + height - 68, 220, 44), CardUi.Muted, 0.75f);
        CardUi.Button(b, Back, "뒤로");
        drawMouse(b);
    }
}
