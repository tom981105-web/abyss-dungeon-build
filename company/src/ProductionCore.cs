...
internal sealed class ProductionCore
{
    ...
    private const int PlanLimit = 12;
    ...
    internal bool TryStart(string recipeKey, int requestedBatches, out string message)
    {
        ...
        if (Context.IsMultiplayer && !Context.IsMainPlayer)
        {
            ...
            message = $"{recipe.DisplayName} 생산계획을 공동 회사에 반영 중입니다.";
            return true;
        }
    ...
    private bool TryConsumeIngredients(ProductionRecipeDefinition recipe, int required, out int inputQualityScore)
    {
        ...
        List<WarehouseStockEntry> candidates = GetIngredientCandidates(recipe).OrderBy(p => p.Quality).ThenBy(p => p.ItemId, StringComparer.OrdinalIgnoreCase).ToList();
    ...
    private void OnTimeChanged(object? sender, TimeChangedEventArgs e)
    {
        ...
        foreach (ProductionJob job in Mod.State.ProductionQueue.ToList())
        {
            ...
            BufferIntermediate(job, recipe.Stages[job.CurrentStageIndex]);
    ...
    private void BufferIntermediate(ProductionJob job, ProductionStageDefinition stage)
    {
        string key = string.IsNullOrWhiteSpace(stage.IntermediateKey) ? $"{job.RecipeKey}:{stage.Key}" : stage.IntermediateKey;
        string display = string.IsNullOrWhiteSpace(stage.IntermediateDisplayName) ? $"{stage.DisplayName} 중간재" : stage.IntermediateDisplayName;
        int quantity = Math.Max(1, job.EstimatedOutputQuantity);
        string stockKey = IntermediateKey(key, job.OutputQuality);
        ...
        job.BufferedIntermediateKey = key;
    ...
    private void Complete(ProductionJob job, ProductionRecipeDefinition recipe)
    {
        ...
        Mod.Company.AddCompanyExperience(Math.Max(1, job.BatchCount) * 3);

        string notice = $"생산 완료: {recipe.DisplayName} {job.OutputGrade}급 {quantity:N0}{recipe.OutputUnit}";
