...
/// <summary>0.8.7 high-resolution art pass drawn after the stable 0.8.6 cleanup layer.</summary>
internal sealed class Production087HiResOverlay
{
    ...
    internal Production087HiResOverlay(ModEntry mod) => Mod = mod;

    internal void Initialize() => Mod.Helper.Events.Display.RenderedActiveMenu += OnRenderedActiveMenu;
    ...
    private bool TryLoad()
    {
        ...
        try
        {
            ...
            if (Atlas.Width < 512 || Atlas.Height < 512)
                throw new InvalidOperationException($"production_visuals_087.png size {Atlas.Width}x{Atlas.Height} is smaller than 512x512.");
            return true;
        }
        catch (Exception ex)
        {
            ...
            {
                Warned = true;
                Mod.Monitor.Log($"0.8.7 hi-res art could not load; keeping the safe 0.8.6 visuals. {ex.Message}", StardewModdingAPI.LogLevel.Warn);
            }
            Atlas = null;
            return false;
        }
    ...
    private void DrawProduction(SpriteBatch b, Production084Menu menu)
    {
        ...
        for (int i = 0; i < Math.Min(3, lines.Count); i++)
        {
            ...
            int machineSprite = line.LineType switch { "Fermentation" => 1, "Packaging" => 2, _ => 0 };
            Rectangle machine = D(t, 42, 244 + i * 126, 130, 98);
            Clear(b, Inflate(machine, S(t, 4)), Cream);
            ...
            {
                Rectangle icon = D(t, 169, 258 + i * 126, 56, 56);
                Clear(b, D(t, 164, 253 + i * 126, 67, 67), Cream);
        ...
        Rectangle topClear = D(t, 566, 207, 108, 99);
        Rectangle topProduct = D(t, 580, 214, 82, 82);
        Clear(b, topClear, Cream2);
        ...
        DrawFlow(b, t, selected, active);
        DrawEmptySlots(b, t);
    }

    private void DrawFlow(SpriteBatch b, UiTransform t, ProductionRecipeDefinition recipe, ProductionJob? active)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            Rectangle clear = D(t, dx + Math.Max(0, ((int)nodeW - 72) / 2), 307, 72, 82);
            Rectangle icon = D(t, dx + Math.Max(0, ((int)nodeW - 66) / 2), 312, 66, 72);
            int sprite = nodes[i].idx switch { -1 => 3, 99 => ProductSpriteIndex(recipe), _ => ProcessSpriteIndex(nodes[i].name) };
            Clear(b, clear, current ? new Color(255, 226, 177) : Cream);
    ...
    private void DrawCatalog(SpriteBatch b, ProductCatalog084Menu catalog)
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            int col = i % 2;
            int row = i / 2;
            Rectangle clear = D(t, 45 + col * 405, 200 + row * 168, 112, 112);
            Rectangle icon = D(t, 54 + col * 405, 207 + row * 168, 94, 94);
            Clear(b, clear, Cream);
            DrawSprite(b, ProductSpriteIndex(recipe), icon, Mod.Production.IsRecipeUnlocked(recipe, out _) ? 1f : 0.40f);
        }

        ProductionRecipeDefinition? selected = Mod.Production.FindRecipe(selectedKey) ?? rows.FirstOrDefault();
        if (selected is not null)
        {
            Rectangle clear = D(t, 1048, 176, 170, 148);
            Rectangle icon = D(t, 1064, 181, 138, 138);
            Clear(b, clear, Cream2);
            DrawSprite(b, ProductSpriteIndex(selected), icon, Mod.Production.IsRecipeUnlocked(selected, out _) ? 1f : 0.44f);
        }
    }

    private void DrawEmptySlots(SpriteBatch b, UiTransform t)
    {
        ...
        if (noIntermediate) DrawSlotStrip(b, t, 34, 716, 640, 72);
        if (noFinished) DrawSlotStrip(b, t, 708, 716, 658, 72);
    }
    ...
    private void DrawSlotStrip(SpriteBatch b, UiTransform t, int x, int y, int w, int h)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            Rectangle ghost = new(slot.X + S(t, 9), slot.Y + S(t, 9), S(t, 34), S(t, 34));
            Fill(b, ghost, new Color(128, 92, 55) * 0.18f);
            float sc = Math.Max(0.18f, t.Scale * 0.60f);
            const string text = "빈 슬롯";
    ...
    private static int ProcessSpriteIndex(string? stage)
    {
        ...
        if (s.Contains("착즙", StringComparison.CurrentCultureIgnoreCase) || s.Contains("압착", StringComparison.CurrentCultureIgnoreCase) || s.Contains("파쇄", StringComparison.CurrentCultureIgnoreCase) || s.Contains("분쇄", StringComparison.CurrentCultureIgnoreCase) || s.Contains("절단", StringComparison.CurrentCultureIgnoreCase)) return 5;
        if (s.Contains("살균", StringComparison.CurrentCultureIgnoreCase) || s.Contains("가열", StringComparison.CurrentCultureIgnoreCase) || s.Contains("숙성", StringComparison.CurrentCultureIgnoreCase) || s.Contains("발효", StringComparison.CurrentCultureIgnoreCase) || s.Contains("염장", StringComparison.CurrentCultureIgnoreCase)) return 6;
        if (s.Contains("병입", StringComparison.CurrentCultureIgnoreCase)) return 7;
        if (s.Contains("포장", StringComparison.CurrentCultureIgnoreCase) || s.Contains("세트", StringComparison.CurrentCultureIgnoreCase)) return 11;
        return 5;
    }
    ...
    private static int ProductSpriteIndex(ProductionRecipeDefinition recipe)
    {
        ...
        if (name.Contains("밀가루", StringComparison.CurrentCultureIgnoreCase) || name.Contains("분말", StringComparison.CurrentCultureIgnoreCase) || name.Contains("가루", StringComparison.CurrentCultureIgnoreCase)) return 14;
    ...
    private static int S(UiTransform t, int v) => Math.Max(1, (int)MathF.Round(v * t.Scale));
    private static Rectangle D(UiTransform t, int x, int y, int w, int h) => new(t.X + (int)MathF.Round(x * t.Scale), t.Y + (int)MathF.Round(y * t.Scale), Math.Max(1, (int)MathF.Round(w * t.Scale)), Math.Max(1, (int)MathF.Round(h * t.Scale)));
    private static Rectangle Inset(Rectangle r, int n) => new(r.X+n, r.Y+n, Math.Max(1,r.Width-n*2), Math.Max(1,r.Height-n*2));
    private static Rectangle Inflate(Rectangle r, int n) => new(r.X-n, r.Y-n, r.Width+n*2, r.Height+n*2);
