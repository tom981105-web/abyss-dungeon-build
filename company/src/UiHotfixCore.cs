...
internal sealed class UiHotfixCore
{
    private const string VersionText = "0.7.4.1";
    ...
    internal UiHotfixCore(ModEntry mod)
    {
        Mod = mod;
    }
    ...
    private void OnUpdateTicked(object? sender, UpdateTickedEventArgs e)
    {
        if (!Context.IsWorldReady || !e.IsMultipleOf(30))
            return;
        FixMenu(Game1.activeClickableMenu);
    }
    ...
    private void OnButtonPressed(object? sender, ButtonPressedEventArgs e)
    {
        ...
        {
            Rectangle productionTab = new(company.xPositionOnScreen + 18, company.yPositionOnScreen + 154, 190, 40);
    ...
    private void OnRenderedActiveMenu(object? sender, RenderedActiveMenuEventArgs e)
    {
        if (Game1.activeClickableMenu is CompanyMenu company)
        {
            DrawCompanyVersionPatch(e.SpriteBatch, company);
            return;
        }
    ...
    private void FixMenu(IClickableMenu? menu)
    {
    ...
    {
        var ui = Game1.uiViewport;
        int desiredX = ui.X + Math.Max(8, (ui.Width - menu.width) / 2);
        int desiredY = ui.Y + Math.Max(8, (ui.Height - menu.height) / 2);
    ...
    {
        var ui = Game1.uiViewport;
        int w = Math.Min(1440, Math.Max(760, ui.Width - 28));
        int h = Math.Min(930, Math.Max(620, ui.Height - 28));
        w = Math.Min(w, Math.Max(620, ui.Width - 8));
        h = Math.Min(h, Math.Max(540, ui.Height - 8));
        int x = ui.X + (ui.Width - w) / 2;
        int y = ui.Y + (ui.Height - h) / 2;

        Rectangle panel = new(x, y, w, h);
        Rectangle header = new(x + 8, y + 8, w - 16, 66);
        Rectangle status = new(x + 8, y + 78, w - 16, 50);
        int bodyTop = y + 136;
        int bodyHeight = Math.Max(250, h - 366);
        int gap = 10;
        int available = Math.Max(600, w - 16 - gap * 2);
        int leftW = (int)(available * 0.30f);
        int centerW = (int)(available * 0.41f);
        int rightW = available - leftW - centerW;
        ...
        int bottomHeight = Math.Max(88, h - (bottomTop - y) - 48);
        int bottomW = (w - 26) / 2;
    ...
    {
        var ui = Game1.uiViewport;
        int w = Math.Min(1180, Math.Max(760, ui.Width - 80));
        int h = Math.Min(860, Math.Max(610, ui.Height - 70));
        w = Math.Min(w, Math.Max(620, ui.Width - 8));
        h = Math.Min(h, Math.Max(540, ui.Height - 8));
        Rectangle panel = new(ui.X + (ui.Width - w) / 2, ui.Y + (ui.Height - h) / 2, w, h);
        SetRectangle(menu, "Panel", panel);
        menu.xPositionOnScreen = 0;
        menu.yPositionOnScreen = 0;
        menu.width = ui.Width;
    ...
    private static void DrawCompanyVersionPatch(SpriteBatch b, CompanyMenu menu)
    {
    ...
    private void DrawProductionIcons(SpriteBatch b, Production2Menu menu)
    {
        ...
        y = finished.Y + 39;
        foreach (ProductStockEntry stock in Mod.State.FinishedGoods.Values.Where(p => p is not null && p.Quantity > 0).OrderByDescending(p => p.Quality).ThenByDescending(p => p.Quantity).Take(4))
        {
    ...
    private static Rectangle LineCard(Rectangle leftPanel, int index)
    {
        ...
        int h = Math.Max(96, (leftPanel.Height - 62) / 3 - 7);
